...
const DEFAULT_CUSTOMER_TYPE_DATA: CustomerTypeDataPoint[] = [
  {
    type: "returning",
...
export function CustomerTypeAnalyticsModal({ data = DEFAULT_CUSTOMER_TYPE_DATA }: CustomerTypeAnalyticsModalProps) {
  const chartData = data.map((item) => ({ label: item.label, value: item.aov }))
        ...
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-border bg-white/70 p-6 shadow-sm">
            <p className="flex items-center gap-2 font-medium text-foreground">
              ...
              <span>AOV by Customer Type</span>
            </p>
                    ...
                    <div key={item.type} className="flex items-center gap-3">
                      ...
                      <div className="flex gap-2">
                        <span className="text-sm font-medium text-foreground">
                          {item.label} ₹{formatNumber(item.aov)}
                        </span>
                      </div>
                    </div>
                  )
          ...
          <div className="flex flex-col gap-6 rounded-2xl border border-border bg-white/70 p-6 shadow-sm">
            <div>
              <p className="flex items-center gap-2 font-medium text-foreground border-b-0">
                ...
                <span>Numbers of customers</span>
              </p>
            </div>
